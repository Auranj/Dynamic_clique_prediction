...
from config import DATASET_CONFIG, MODEL_CONFIG, TRAIN_CONFIG, RESULTS_CONFIG

# 使用配置中的目录
RESULTS_BASE_DIR = RESULTS_CONFIG['base_dir']
TRAIN_RESULTS_DIR = RESULTS_CONFIG['train_dir']
...
def train_model():
    ...
    loader = DataLoader(
        ...
        overlap_ratio=DATASET_CONFIG.get('overlap_ratio', 0.5),
        adaptive_window=DATASET_CONFIG.get('adaptive_window', False)
    )
    ...
    if DATASET_CONFIG.get('adaptive_window', False):
        ...
        plt.grid(True)
        plt.savefig(os.path.join(TRAIN_RESULTS_DIR, 'adaptive_windows.png'), dpi=300, bbox_inches='tight')
        ...
        window_df.to_csv(os.path.join(TRAIN_RESULTS_DIR, 'adaptive_windows.csv'), index=False)
    ...
    for epoch in range(TRAIN_CONFIG['num_epochs']):
        ...
        if save_model:
            ...
            train_hist_df.to_csv(os.path.join(TRAIN_RESULTS_DIR, 'train_history.csv'), index=False)
            val_hist_df.to_csv(os.path.join(TRAIN_RESULTS_DIR, 'val_history.csv'), index=False)
        else:
            patience_counter += 1
    ...
    print(f"\n训练完成，结果已保存到 {TRAIN_RESULTS_DIR} 目录")
